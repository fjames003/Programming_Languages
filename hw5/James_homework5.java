...
class BinOp implements AExp {
    ...
    protected Op op;
    BinOp(AExp left, Op op, AExp right) {
      this.left = left;
      this.right = right;
      this.op = op;
    }
...
class Calculate implements AInstr {
    ...
    public void eval(Stack<Double> stack) {
      if(stack.size() >= 2) {
        Double n1 = stack.pop();
        Double n2 = stack.pop();
        stack.push(this.op.calculate(n2, n1));
      } else {
        throw new RuntimeException("To 'Calculate', two items must be in the stack, stack size = " + stack.size());
      }
    }
    public String toString() {
...
class CalcTest {
	...
	AExp aexp1 =
	    new BinOp(new BinOp(new Num(1.0), Op.PLUS, new Num(2.0)),
		      Op.TIMES,
		      new Num(4.0));
	System.out.println("aexp1   evaluates to: " + aexp1.eval()); // aexp evaluates to 12.0

  AExp aexp2 =
      new BinOp(new BinOp(new Num(1.0), Op.MINUS, new Num(2.0)),
          Op.TIMES,
          new Num(4.0));
  System.out.println("aexp2   evaluates to: " + aexp2.eval()); // aexp evaluates to -4.0
	...
	Instrs instrs1 = new Instrs(is);
	System.out.println("instrs1 evaluates to: " + instrs1.eval());  // instrs evaluates to 12.0

  List<AInstr> is1 = new LinkedList<AInstr>();
  is1.add(new Calculate(Op.PLUS));
  Instrs instrs3 = new Instrs(is1);
  try {
    instrs3.eval();
  } catch (RuntimeException e) {
      System.out.println("instrs3 evaluates to: " + e);  // instrs evaluates to 12.0
  }

  //Non-cummative test (4.0 - 2.0) / 4.0 = 0.5, if evaluting wrong could be 2.0 (messing up division) or -2.0 (messing up both)
  ...
  System.out.println("instrs2 evaluates to: " + instrs2.eval());  // instrs evaluates to 0.5
